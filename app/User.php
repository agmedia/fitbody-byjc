...
class User extends Authenticatable
{
    ...
    {
        $role = isset($this->request->user_role) ? $this->request->user_role : 'customer';
        ...
        $stored = $this->insertGetId([
            ...
            'role'       => $role,
            'status'     => (isset($this->request->user_status) and $this->request->user_status == 'on') ? 1 : 0,
            'created_at' => Carbon::now(),
            'updated_at' => Carbon::now()
        ]);

        $user = User::find($stored);
